...
(function (PlatformType) {
    ...
    PlatformType[PlatformType["BINANCENEW"] = 10] = "BINANCENEW";
...
class BaseController {
    ...
    showPlatformOrders(_orders, platform) {
        switch (platform) {
            ...
            case PlatformType.BINANCENEW:
                return this.filterOrders(_orders.filter(p => p.Platform.toUpperCase() == 'BINANCENEW'));
        }
        return null;
    }
    /**
...
let CoinTrackingApp = CoinTrackingApp_1 = class CoinTrackingApp extends lit_element_s {
  ...
  `;
    ...
    showNewBinace() {
        if (this._ordersBinance.length == 0)
            this._ordersBinance = this._baseController.showPlatformOrders(this._allOrders, PlatformType.BINANCENEW);
        this._orders = this._ordersBinance;
        this.requestUpdate();
    }
    showHotBit() {
    ...
    <div class="row">
      <ul style="display:flex;margin-left:50px;">
        ...
        <li class="col-2">
          <a class="nav-link" href="#"  @click="${this.showNewBinace}">N Binance</a>
        </li>
        <li class="col-2">
          <a class="nav-link" href="#"  @click="${this.showBinace}">Binance</a>
        </li>
